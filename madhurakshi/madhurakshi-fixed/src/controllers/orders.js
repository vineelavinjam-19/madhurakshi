...
// ── POST /api/orders/cod ──────────────────────────────────────
// All work (stock check, stock decrement, order insert, order_items insert,
// cart clear) runs inside the place_cod_order Postgres function (migration 002)
// so it is fully atomic — no window where stock is decremented but the order
// doesn't exist, and no need for a manual rollback on the JS side.
export async function placeCodOrder(req, res) {
  try {
    const { shipping_address } = req.body;
    const { cartItems, totalAmount } = await fetchCart(req.user.id);

    // Build the items payload the DB function expects
    const items = cartItems.map(item => {
      const fabricExtra = resolveFabricExtra(item.products.fabric_options, item.fabric_option);
      return {
        product_id:         item.products.id,
        ...
        fabric_option:      item.fabric_option,
        fabric_extra_price: fabricExtra,
        unit_price:         parseFloat(item.products.price) + fabricExtra,
      };
    });

    const { data: rows, error: rpcErr } = await supabase.rpc('place_cod_order', {
    ...
    if (!result?.ok) {
      // Stock check failed — surface which products are short
      const failed = (result?.failed_stock || []).map(f => {
        const item = cartItems.find(c => c.products.id === f.product_id);
        return item
          ? `"${item.products.name}" (${f.available} left)`
          : f.product_id;
      });
      const e = new Error(`Insufficient stock for: ${failed.join(', ')}`);
      e.status = 409;
      throw e;
    }
