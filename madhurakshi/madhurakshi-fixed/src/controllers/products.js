...
export async function deleteProduct(req, res) {
  try {
    const { data, error } = await supabase
      ...
      .delete()
      .eq('id', req.params.id)
      .select('id')   // returns the deleted row — empty array means nothing was found
      .single();

    if (error) {
      // PGRST116 = "JSON object requested, multiple (or no) rows returned"
      // i.e. the .single() found nothing — product didn't exist
    ...
    res.json({ message: 'Product deleted', id: data.id });
  } catch (err) {
